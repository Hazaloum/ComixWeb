...
export function AnimatedTetrahedron() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw points with cyan to emerald gradient
      points.forEach((point) => {
        const alpha = 0.15 + (point.z + 1.5) * 0.25;
        const gradientFactor = (point.y - (centerY - scale)) / (scale * 2);
        const r = Math.round(8 + gradientFactor * 8);
        const g = Math.round(145 - gradientFactor * 30);
        const b = Math.round(178 - gradientFactor * 49);
